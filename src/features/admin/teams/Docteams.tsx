// src/features/admin/teams/Docteams.tsx
...
const Docteams: React.FC = () => {
  ...
  const handleCardClick = (team: any) => {
    navigate(
      `/admin/docteams/${team.teamId || team.id}?teamName=${encodeURIComponent(team.name)}&teamLead=${encodeURIComponent(team.teamLeadName || 'Not Assigned')}&rating=8`
    );
  };
  ...
  if (loading) {
    return (
      <div className="w-full h-72 flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-100 shadow-xs">
        ...
        <p className="text-xs font-bold text-slate-700">Loading Department Teams...</p>
        <p className="text-[11px] text-slate-400 mt-0.5">Fetching latest records...</p>
      </div>
    );
  }
      ...
      <div className="w-full bg-white rounded-2xl shadow-xs border border-slate-100 p-5 sm:p-6">
        <div className="flex items-start gap-3.5 p-4 bg-rose-50/80 border border-rose-200 rounded-xl">
          ...
          <div className="flex-1">
            <h4 className="font-bold text-sm text-rose-900">Failed to load department squads</h4>
            <p className="text-xs text-rose-700 mt-0.5">{error}</p>
            <button 
              onClick={() => departmentId && dispatch(fetchTeamsByDepartment(departmentId))}
              className="mt-2.5 inline-flex items-center gap-1.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition shadow-xs"
            >
  ...
  if (departmentTeams.length === 0 && !loading) {
    return (
      <div className="w-full bg-white rounded-2xl shadow-xs border border-slate-100 p-6 sm:p-8 flex flex-col items-center justify-center text-center">
        ...
        <h3 className="text-base font-bold text-[#1b2559]">No Teams Available</h3>
        <p className="text-xs text-slate-400 max-w-xs mt-1">
          There are no teams configured for this department.
        </p>
      </div>
    );
  }
        ...
        {departmentTeams.map((team) => {
          ...
          return (
            ...
            >
              ...
              {/* VISUAL TOP BLOCK - Tight & Compact */}
              ...
              <div className="p-3.5 flex-1 flex flex-col justify-between gap-2.5 bg-white">
                ...
                <div className="flex items-center justify-between gap-1">
                  ...
                  <span className="text-[10px] font-semibold text-slate-400 truncate max-w-[120px]">
                    {team.departmentName?.split(' ')[0] || 'Department'}
                  </span>
                </div>
